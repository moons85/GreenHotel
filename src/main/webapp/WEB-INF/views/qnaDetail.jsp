    ...
    <div class="container">
        <div class="row no-gutters">
            ...
            <div class="col-md-12 p-4 p-md-5 bg-white">
                ...
                <h3 class="font-weight-bold mb-4 h2_title h2_title2">Q. ${faq.title}</h3>
                ...
                <div class="text-left textdiv">
                    ...
                    <div class="reviewtext text-right">
                        작성일:
                        <fmt:formatDate value="${faq.createTime}" pattern="yyyy.MM.dd" type="date"/><br>
                    </div>
                </div>
            ...
            <c:forEach var="faqreply" items="${faqreplys}">
                <c:if test="${faqreply.faq.id==faq.id}">
                    <div class="col-md-12 p-4 p-md-5 bg-primary">
                        <div class="text-left textdiv">
                            작성자: ${faqreply.user.name}<br>
                            <br>
                            <div class="reviewtext text-light">
                                <h3 class="font-weight-bold mb-4 h2_title h2_title2 text-light">A. ${faqreply.faq.title}</h3>
                                <p>${faqreply.content}</p>
                            </div><br>
                            <div class="reviewtext text-right text-light">
                                작성일:
                                <fmt:formatDate value="${faqreply.createTime}" pattern="yyyy.MM.dd" type="date"/><br>
                            </div>
                        </div>
                        <input type="hidden" value="${faqreply.id}" id="frId">
                        <c:if test="${principal.user.roleType=='MANAGER' || principal.user.id==faq.user.id}">
                            <button onclick="deleteById()">댓글 삭제</button>
                        </c:if>
                    </div>
                </c:if>
            </c:forEach>
            <c:if test="${principal.user.roleType=='MANAGER' || principal.user.id==faq.user.id}">
                <div class="col-md-12 p-4 p-md-5 bg-primary">
                    <div class="text-left textdiv">
                        <br>
                        <div class="reviewtext text-light">
                            <input type="hidden" value="${faq.id}" id="fId">
